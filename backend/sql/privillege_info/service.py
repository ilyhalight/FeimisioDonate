...
class DbPrivillegeInfoService:
    def __init__(self) -> None:
        self.database_sc = 'fd_privillege_info' # Название таблицы в базе данных (для логов)
    ...
    async def init_table(self):
        ...
        try:
            async with db.cursor() as cursor:
                await cursor.execute("""CREATE TABLE IF NOT EXISTS `fd_privillege_info` (
    ...
    async def add(self, link: str, name: str, desc: str, img_link: str = None, is_big_img: bool = None, img_reverse_side: bool = None):
        ...
        try:
            async with db.cursor() as cursor:
                ...
                await cursor.execute('INSERT INTO `fd_privillege_info` (`link`, `name`, `desc`, `img_link`, `is_big_img`, `img_reverse_side` VALUES (%s)', (link, name, desc, img_link, is_big_img, img_reverse_side))
                await db.commit()
                return True
        except AttributeError as err:
            log.error(f'Failed connection to database: {err}')
            ...
            log.exception(f'Failed to add a privillege information to the database ({self.database_sc}): {err}')
            return None
        finally:
            db.close() if db else None
    ...
    async def get(self, link: str = None):
        ...
        try:
            ...
            log.exception(f'Failed to get privillege info (link: {link}) to the database ({self.database_sc}): {err}')
            return None
        finally:
            db.close() if db else None
    ...
    async def remove(self, uid: int):
        ...
        try:
            async with db.cursor() as cursor:
                await cursor.execute('DELETE FROM `fd_privillege_info` WHERE `uid` = %s', (uid, ))
                await db.commit()
                return True
        except AttributeError as err:
            log.error(f'Failed connection to database: {err}')
            ...
            log.exception(f'Failed to remove privillege information to the database ({self.database_sc}): {err}')
            return None
        finally:
            db.close() if db else None