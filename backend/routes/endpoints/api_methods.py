...
@router.post('/api/payments-methods', response_class = JSONResponse, summary = 'Redirect to another page', responses = payment_methods_responses)
async def index(steam_link: str = Form(), uid: int = Form(), aggregator: str = Form()):
    ...
    if aggregator in ['freekassa', 'enot', 'crystalpay'] and uid > 0:
        ...
        if privillege:
            ...
            if discount > 0 and price_clear > 0:
        ...
        if price > 0:
            await MassLog().info(f'[Пользователь]({steam_link}) создал ссылку на оплату **{privillege_name}** \(UID: **{uid}**\) за **{price}** руб\. \(скидка: **{discount}**% \| платежка: {aggregator}\)')
            if 'id' in steam_link:
            ...
            if aggregator != 'crystalpay':
                return RedirectResponse(url = f'http://localhost:3999/payments/{aggregator}?steam={steam_arr}&price={price}&uid={uid}', status_code = status.HTTP_303_SEE_OTHER)
            else:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get('https://api.crystalpay.ru/v1/', params = {
                        ...
                        await MassLog().error(f'Ошибка при создании ссылки на оплату через CrystalPay: {str(data)}')
                        return JSONResponse(content = {'error': 'Server error'}, status_code = status.HTTP_500_INTERNAL_SERVER_ERROR)
        else:
            await MassLog().info(f'[Пользователь]({steam_link}) получил бесплатную привилегию {privillege_name}')
